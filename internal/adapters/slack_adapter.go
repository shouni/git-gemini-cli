...
func (s *SlackAdapter) Notify(ctx context.Context, publicURL, storageURI string, reg domain.ReviewRequest) error {
	...
	content := s.buildSlackContent(publicURL, storageURI, reg)
...
func (s *SlackAdapter) buildSlackContent(publicURL, storageURI string, reg domain.ReviewRequest) string {
	cfg := reg.Config
