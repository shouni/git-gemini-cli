...
| :--- | :--- | :--- |
| **`detail`** | **[`adapters/prompt_detail.md`](https://github.com/shouni/git-gemini-cli/blob/main/internal/adapters/prompt_detail.md)** | **コード品質と保守性の向上**を目的とした詳細なレビュー。可読性、重複、命名規則、一般的なベストプラクティスからの逸脱など、広範囲な技術的側面に焦点を当てます。 |
| **`release`** | **[`adapters/prompt_release.md`](https://github.com/shouni/git-gemini-cli/blob/main/internal/adapters/prompt_release.md)** | **本番リリース可否の判定**を目的としたクリティカルなレビュー。致命的なバグ、セキュリティ脆弱性、サーバーダウンにつながる重大なパフォーマンス問題など、リリースをブロックする問題に限定して指摘します。 |
